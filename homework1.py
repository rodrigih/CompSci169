...
def bracketMinimum(x1,step,gamma,func):
	...
	Return:
		...
		all the points found where x1 < x2 < x3 and a second one with the values
		of the function evaluated at those points
	'''
	x2 = x1 + step
...
def goldenSearch(start,step,func,max_iter):
	'''
	Description:
		This function finds the minimum of the function given by using the
		golden section method. It then prints out the results along with 
	...
	Arguments:
		...
		gamma(float): the rate in which to use to find the third point
		func(function): function chosen to minimize
		max_iter(int): the maximum number of times the function is to be calculated
...
def main():
	exec(readFunction("input1.txt"),globals())
